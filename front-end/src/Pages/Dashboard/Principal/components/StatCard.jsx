...
/**
 * StatCard Component for Principal Dashboard
 * Displays key statistics with icon, title, value, and subtitle
 * Matches the design system used across all dashboards
 * @param {string} title - The card title
 * @param {string} value - The main statistic value
 * @param {React.Component} icon - The icon component
 * @param {string} color - Color theme (blue, orange, green, red, purple)
 ...
 * @param {string} suffix - Optional suffix for the value (e.g., "%")
 */
export default function StatCard({
  ...
  if (onClick) {
    return (
      <button onClick={onClick} className="w-full text-left focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 rounded-lg">
        <CardContent />
      </button>
    )
  }
