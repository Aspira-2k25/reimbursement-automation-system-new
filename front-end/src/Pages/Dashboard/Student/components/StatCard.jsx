...
const iconMap = {
  ...
  BuildOutlined: (props) => <Wrench {...props} className="text-blue-600" />,
...
export default function StatCard({ option, onApply }) {
  ...
  const Icon = iconMap[option.icon] || iconMap.SchoolOutlined
  ...
  return (
    <div className="group relative overflow-hidden rounded-xl sm:rounded-2xl bg-white border border-[color:var(--color-moss-sage)]/50 shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
      {/* Hover background effect */}
      <div className="absolute inset-0 bg-gradient-to-br from-blue-50/50 to-indigo-50/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
      
      <div className="relative p-4 sm:p-5 lg:p-6 h-full flex flex-col">
        {/* Icon container with hover animation */}
        <div className="flex h-10 w-10 sm:h-12 sm:w-12 lg:h-14 lg:w-14 items-center justify-center rounded-lg sm:rounded-xl mb-3 sm:mb-4 group-hover:scale-110 transition-transform duration-300" style={{background: 'linear-gradient(135deg, var(--color-moss-lime), white)'}}>
        ...
        {/* Card title */}
        <h3 className="font-semibold text-base sm:text-lg lg:text-xl mb-2" style={{color: 'var(--color-moss-deep)'}}>{title}</h3>
        
        {/* Card description */}
        <p className="text-xs sm:text-sm lg:text-base flex-1 leading-relaxed" style={{color: 'color-mix(in oklab, var(--color-moss-deep) 65%, white)'}}>{description}</p>
        ...
        <button
          ...
          style={{background: 'linear-gradient(135deg, var(--color-moss-olive), var(--color-moss-lime))'}}
        >
          Apply Now
        </button>
      </div>
