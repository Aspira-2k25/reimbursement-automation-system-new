 ...
 * Mobile-First & Dynamic: Maps approval stages: Student/Faculty -> Coordinator -> HOD -> Principal -> Accounts (Disbursed)
 */
export const ApprovalTimeline = ({ 
  ...
  const getStepIndex = (currentStatus) => {
    ...
    if (s.includes('approved')) return steps.length - 1;
  ...
  return (
    <div className="w-full rounded-2xl border border-slate-100 bg-white/90 p-4 sm:p-5 shadow-sm backdrop-blur-sm">
      <div className="mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div>
          <h4 className="text-xs sm:text-sm font-bold text-slate-800 tracking-tight">Approval Pipeline</h4>
          <p className="text-[10px] sm:text-xs text-slate-500">Live administrative tracking status</p>
        </div>
        <div>
          <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] sm:text-xs font-semibold ${
            isRejected 
            ...
            {isRejected ? (
              <><XCircle className="h-3 w-3 sm:h-3.5 sm:w-3.5" /> Rejected by {rejectedBy || 'Authority'}</>
            ) : status === 'Reimbursed' ? (
              <><CheckCircle2 className="h-3 w-3 sm:h-3.5 sm:w-3.5" /> Payment Disbursed</>
            ) : (
              <><Clock className="h-3 w-3 sm:h-3.5 sm:w-3.5 animate-pulse" /> {status}</>
            )}
          </span>
        </div>
      </div>
      ...
      <div className="relative flex items-center justify-between pt-1 pb-1">
        {/* Connecting Line */}
        <div className="absolute top-1/2 left-3 right-3 sm:left-4 sm:right-4 -translate-y-1/2 h-1 bg-slate-100 -z-0">
          <div 
        ...
        {steps.map((step, idx) => {
          ...
          return (
            <div key={step.key} className="relative z-10 flex flex-col items-center">
              <div 
                className={`flex h-7 w-7 sm:h-9 sm:w-9 items-center justify-center rounded-full border-2 transition-all duration-300 ${
                  isPassed
                    ? 'border-[#3B945E] bg-[#3B945E] text-white shadow-md shadow-[#3B945E]/20'
                    : isCurrent
                    ? 'border-[#3B945E] bg-white text-[#3B945E] ring-2 sm:ring-4 ring-[#65CCB8]/30 animate-pulse'
                    : isStepRejected
                    ? 'border-rose-500 bg-rose-500 text-white ring-2 sm:ring-4 ring-rose-200'
              ...
              <span className={`mt-1.5 text-[10px] sm:text-xs font-semibold text-center whitespace-nowrap ${
                isCurrent ? 'text-[#3B945E]' : isPassed ? 'text-slate-700' : 'text-slate-400'
              }`}>
                {step.label}
              </span>
              <span className="hidden sm:block text-[9px] sm:text-[10px] text-slate-400 text-center">{step.role}</span>
            </div>
          );
        })}
      </div>
        ...
        <div className={`mt-4 rounded-xl p-3 text-xs ${
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="mt-0.5 text-slate-600">{rejectionRemarks || accountsRemarks}</p>
            </div>
          </div>
        </div>
      )}
