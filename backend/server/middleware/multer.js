...
// Maximum file sizes (1MB limit per file)
const MAX_FILE_SIZE = 1 * 1024 * 1024; // 1MB per file
const MAX_TOTAL_SIZE = 2 * 1024 * 1024; // 2MB total for all files
...
const validateTotalUploadSize = (req, res, next) => {
  ...
  if (contentLength > MAX_TOTAL_SIZE) {
    return res.status(413).json({
      ...
      message: `Total upload size exceeds 1MB limit per file (2MB total)`
    });
  }
